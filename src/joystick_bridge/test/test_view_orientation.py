"""Tests for manual operator-frame direction conversion."""
...
def test_dpad_selects_absolute_estop_direction():
...
@pytest.mark.parametrize(
    ...
    [
        (VIEW_FRONT, 0.0),
        (VIEW_RIGHT, -math.pi / 2.0),
        (VIEW_BACK, math.pi),
        (VIEW_LEFT, math.pi / 2.0),
    ],
)
...
def test_operator_direction_rotation_preserves_magnitude_independent_angle():
    ...
    assert math.cos(actual) == pytest.approx(math.cos(math.pi))
    assert math.sin(actual) == pytest.approx(math.sin(math.pi))
