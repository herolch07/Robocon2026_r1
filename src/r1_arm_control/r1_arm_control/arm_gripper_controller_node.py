...
class ArmGripperControllerNode(Node):
    """
    Arm gripper safety/control node.

    Subscribes:
      /arm_gripper_speed_cmd: Float32MultiArray [speed_rad_s]

    Publishes:
      /damiao_control: Float32MultiArray [motor_id, mode, speed_rad_s, duration]
      /arm_gripper_status: Float32MultiArray [target_speed, commanded_speed, timeout, motor_id]
    """
    ...
    def __init__(self):
        ...
        self.status_pub = self.create_publisher(Float32MultiArray, "/arm_gripper_status", 10)
        ...
        self.get_logger().info(f"Arm gripper motor ID: {self.motor_id}")
    ...
    def gripper_command_callback(self, msg):
        if not msg.data:
            self.get_logger().warn("Invalid /arm_gripper_speed_cmd: expected [speed_rad_s]")
            return
