...
class HorizontalControllerNode(Node):
    """
    Arm horizontal movement safety/control node.

    Subscribes:
      /horizontal_speed_cmd: Float32MultiArray [speed_rad_s]

    Publishes:
      /damiao_control: Float32MultiArray [motor_id, mode, speed_rad_s, duration]
      /horizontal_status: Float32MultiArray [target_speed, commanded_speed, timeout, motor_id]
    """
    ...
    def __init__(self):
        ...
        self.command_sub = self.create_subscription(
            ...
            self.horizontal_command_callback,
            10,
        )
        self.motor_pub = self.create_publisher(Float32MultiArray, "/damiao_control", 10)
        self.status_pub = self.create_publisher(Float32MultiArray, "/horizontal_status", 10)

        publish_hz = max(float(self.get_parameter("publish_hz").value), 1.0)
        self.period = 1.0 / publish_hz
        self.timer = self.create_timer(self.period, self.timer_callback)

        self.get_logger().info("Horizontal controller initialized")
        self.get_logger().info(f"Horizontal motor ID: {self.motor_id}")
    ...
    def horizontal_command_callback(self, msg):
        if not msg.data:
            self.get_logger().warn("Invalid /horizontal_speed_cmd: expected [speed_rad_s]")
            return
